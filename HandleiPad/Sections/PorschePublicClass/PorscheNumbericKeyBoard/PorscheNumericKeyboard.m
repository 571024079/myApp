...
static CGRect psnk_get_frame()
{
    CGFloat width = MIN([UIScreen mainScreen].bounds.size.width,
                        [UIScreen mainScreen].bounds.size.height);
    CGFloat height = ((NSUInteger)(width * .32 + 30)) * 1;
    return CGRectMake(0, 0, width, height);
}

static UIImage *psnk_image(UIColor *color, CGSize size)
{
...
static UIImage *psnk_delete_icon()
{
...
/*
static UIImage *psnk_keyboard_icon()
{
    CGFloat scale = [UIScreen mainScreen].scale;
...
@interface UIView (PSNKFindFirstResponder)

+ (UIView *)psnk_firstResponder;

@end


@implementation UIView (PSNKFindFirstResponder)

+ (UIView *)psnk_firstResponder
{
    return [[[UIApplication sharedApplication] keyWindow] psnk_findFirstResponder];
}

- (UIView *)psnk_findFirstResponder
{
    if ([self isFirstResponder]) return self;
    
    for (UIView *view in self.subviews) {
        UIView *responder = [view psnk_findFirstResponder];
...
@class PorscheNumericKeyboardItem;

@interface PorscheNumericKeyboardItem : UIButton
...
@implementation PorscheNumericKeyboardItem

- (void)setBackgroundColor:(UIColor *)backgroundColor forState:(UIControlState)state
{
    [self setBackgroundImage:psnk_image(backgroundColor, CGSizeMake(1, 1))
                    forState:state];
}

@end

@interface PorscheNumericKeyboard () <UIInputViewAudioFeedback>
{
    NSTimer *deleteTimer;
}
...
@implementation PorscheNumericKeyboard
...
- (void)_initUI
{
    ...
    for (int i = 0; i < 14; ++i) {
        PorscheNumericKeyboardItem *item = [PorscheNumericKeyboardItem buttonWithType:UIButtonTypeCustom];
        if (i == 10) {
            item.tag = 46;
            ...
            item.tag = 127;
            [item setImage:psnk_delete_icon() forState:UIControlStateNormal]; //删除按钮
            ...
            item.tag = -1;
//            [item setImage:psnk_keyboard_icon() forState:UIControlStateNormal];//键盘按钮
             [item setImage:psnk_delete_icon() forState:UIControlStateNormal];
            UILongPressGestureRecognizer *g = [[UILongPressGestureRecognizer alloc] initWithTarget:self
                                                                                            action:@selector(deleteItemLongPress:)];
            [item addGestureRecognizer:g];
...
- (void)_clickedDelete
{
    ...
    if ([delegate respondsToSelector:@selector(textField:shouldChangeCharactersInRange:replacementString:)]) {
        NSRange range = [self _selectedRangeInInputView:(id<UITextInput>)self.firstResponder];
        if ([delegate textField:(UITextField *)self.firstResponder shouldChangeCharactersInRange:range replacementString:@""]) {
...
//    CGFloat height = CGRectGetHeight(self.bounds) / 4.f - .5f;
    ...
    CGFloat width = CGRectGetWidth(self.bounds) / 3.f - .5f;
    CGFloat height = CGRectGetHeight(self.bounds) / 4.f - .5f;
...
- (void)buttonTouchUpInside:(PorscheNumericKeyboardItem *)sender
{
    if (!self.firstResponder || ![self.firstResponder isFirstResponder]) {
        self.firstResponder = (UIView<UIKeyInput>*)[UIView psnk_firstResponder];
