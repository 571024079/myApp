...
@interface VinCameraViewController ()<UIAlertViewDelegate,AVCaptureVideoDataOutputSampleBufferDelegate,ScanInfoTableViewDelegate>{
    ...
    ScanInfoTableView *_scanTableView;
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeOrient:) name:UIApplicationDidChangeStatusBarOrientationNotification object:nil];
...
- (void) initCameraAndLayer
{
    ...
    //设置vin码的检测区域
//    [_vinTyper setVinRegionWithLeft:350 Top:247 Right:1025-100 Bottom:472];
    [_vinTyper setVinRegionWithLeft:350 Top:247 Right:1025-100 Bottom:472];

//    UIView *view = [[UIView alloc] initWithFrame:centerRect];
...
//    [self.view addSubview:_topView.promptLabel2];
    ...
    _topView.label = [[UILabel alloc] initWithFrame:CGRectMake(0, 80, 1024, 60)];
    _topView.label.text = @"";
...
//    [self.view addSubview:_topView.scan_line];
    CGPoint top = center;
    top.y -= _topView.centerRect.size.height/2;
    [_topView.scan_line setCenter:top];
    [self.view addSubview:_topView.scan_line];
    linePoint = _topView.scan_line.center;
    
    _topView.resultImg = [[UIImageView alloc] initWithFrame:CGRectMake((1024 - 355) /2.0,50, 355, 100)];
...
- (void)initScanInfoViewWithInfo:(NSString *)info {
    
    if (!info.length) {
        return;
    }
    _scanTableView = [[ScanInfoTableView alloc] initWithFrame:CGRectMake(0, 0, 450, 185)];
    _scanTableView.center = CGPointMake(self.view.center.x, 300);
    _scanTableView.delegate = self;
    
    _scanTableView.vinTextField.text = info;
    _scanTableView.addressLabel.text = @"";
    _scanTableView.carNumTextFied.text = @"";
...
- (void)scanInfoTableViewCancleClick //取消
{
    ...
    [_scanTableView removeFromSuperview];
    //单指单击
    _isRecognize = YES;
    _topView.label.text = @"";
    _topView.promptLabel1.text = @"";
...
- (void)scanInfoTableViewConfilrmWidth:(ScanInfoModel *)model //完成
{
    if (self.Finished)
    {
        self.Finished(_scanTableView.carNumTextFied.text, _scanTableView.addressLabel.text, _scanTableView.vinTextField.text);
    }
...
- (void)scanInfoTableViewcarCardastralWillShow {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        _scanTableView.center = CGPointMake(_scantableViewCenter.x, _scantableViewCenter.y - 50);
...
- (void)scanInfoTableViewcarCardastralWillDismiss {
    ...
    [UIView animateWithDuration:0.3 animations:^{
        _scanTableView.center = _scantableViewCenter;
...
-(void)moveScanline{
    ...
    [UIView animateWithDuration:2.5f delay:0.0f options:UIViewAnimationOptionRepeat animations:^{
        ...
        center.y += _topView.centerRect.size.height;
...
//vin获取车辆信息
- (void)getVinInfoWithVIN:(NSString *)VIN {
//    WeakObject(self);
    [PorscheRequestManager carMessageWithVIN:VIN carComplete:^(id model, NSError *error) {
        PResponseModel *responser = (PResponseModel *)model;
        if (!error)
        {
            PorscheNewCarMessage *carMessage = [PorscheNewCarMessage yy_modelWithDictionary:responser.object];
            _scanTableView.carNumTextFied.text = carMessage.ccarplate;
            _scanTableView.addressLabel.text = carMessage.plateplace;
            
//            if([carMessage.woid integerValue])
//            {
//                [HDStoreInfoManager shareManager].carorderid = carMessage.woid;
//            }
        }
    }];
}
